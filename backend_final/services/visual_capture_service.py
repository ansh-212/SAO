...
from services.ai_service import LANGUAGE_NAMES, get_client, types
...
CAPTURE_PATTERN = re.compile(
    r"(draw|diagram|flow\s?chart|figure|sketch|illustrate|white\s?paper|whiteboard|write the steps|write steps|commands?|command sequence|workflow|architecture|block diagram|process flow|formula)",
    re.IGNORECASE,
)
...
def enrich_question_for_capture(question: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if capture_required:
        enriched["capture_hint"] = (
            "Use Capture to upload a photo of your handwritten response. "
...
def _fallback_visual_evaluation(question: Dict[str, Any], typed_context: str = "") -> Dict[str, Any]:
    ...
    if note_text:
        ...
        feedback = (
            "Image uploaded successfully. A typed note was used as fallback context because multimodal evaluation is not available right now. "
            "Add a GEMINI_API_KEY for full automatic analysis."
        )
    else:
        summary = "Handwritten answer image captured successfully."
        feedback = (
            "Image uploaded successfully, but automatic interpretation is limited without Gemini multimodal access. "
...
def evaluate_visual_capture_payload(
    ...
    prompt = f"""You are evaluating a student's handwritten interview answer.
...
def evaluate_interview_capture(
    question_text: str,
    image_path: str,
    language: str = "en",
    typed_context: str = "",
    ...
    question_payload = {
        ...
        "bloom_level": "analyze",
        "rubric": {
            "depth": "Covers complete reasoning or structure",
            "accuracy": "Technical correctness of symbols/steps/diagram",
            "application": "Relevance to the asked interview problem",
            "originality": "Clear and structured explanation",
        },
    }
    return evaluate_visual_capture_payload(
