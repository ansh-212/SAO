  ...
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        gradient:
          'bg-gradient-to-r from-primary to-accent text-white shadow-lg shadow-primary/30 hover:opacity-95',
      },
      size: {
...
/* `data-variant` is forwarded so `dashboard-dark.css` (scoped to `.dark-app`)
 * can apply the gradient/ghost/outline button look from the AI Interview
 * Coach design system without re-writing every page. */
  ...
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn('dk-glass-btn', buttonVariants({ variant, size, className }))}
        ref={ref}
        {...props}
      />
    )
